...
const SignIn = () => {
    ...
    return (
        ...
        <form className=' space-y-2 text-gray-500 h-4/6 flex justify-center flex-col items-center'>
            ...
            <input type='password'  name='email 'className='mt-2 p-2 bg-blue-200 focus:outline-none border-none pl-5  rounded-lg' />     
            </div>

            <div className='flex flex-col w-1/2'>
            <label>confirm password</label> 
            <input type='password' autoComplete name='password ' className='mt-2 mt-5 p-2 bg-blue-200 focus:outline-none border-none pl-5  rounded-lg' />     
            </div>
            
            <button type='submit' className=' text-lg bg-gray-700 block w-1/2 rounded-xl p-2 font-bold text-white'>send</button>
            <button onClick={ signInWithGoogle} className=' text-lg bg-gray-700 block w-1/2 rounded-xl p-2 font-bold text-white'>sign up with Google</button>
        </form>
        </div>
    )
}
